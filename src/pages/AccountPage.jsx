...
const STATUS_STEPS = [
  { key: "pending", label: "account.statusPending", icon: (
  ...
  if (isCancelled) {
    return (
      <div className="flex items-center gap-2 px-3 py-1.5 bg-red-50 dark:bg-red-500/10 rounded-full w-fit">
        ...
        <span className="text-[11px] font-semibold text-red-600 dark:text-red-400">{t("account.statusCancelled")}</span>
      </div>
    );
  }
...
export default function AccountPage() {
  ...
  const handlePasswordChange = async (e) => {
    e.preventDefault();
    const errs = {};
    if (!passwordForm.currentPassword) errs.currentPassword = t("account.currentPasswordRequired");
    if (!passwordForm.newPassword) errs.newPassword = t("account.newPasswordRequired");
    else if (passwordForm.newPassword.length < 6) errs.newPassword = t("common.passwordMinLength");
    if (passwordForm.newPassword !== passwordForm.confirmPassword) errs.confirmPassword = t("auth.passwordsMismatch");
    if (Object.keys(errs).length > 0) { setFormErrors(errs); return; }
    setSaving(true);
    try {
  ...
  const handleCancelOrder = async (orderId) => {
    try {
      ...
      toast.success(t("account.orderCancelled"));
    } catch (err) {
      toast.error(err.message || t("account.orderCancelFailed"));
    }
  };
  ...
  return (
    <Layout hideSideNav>
      <div className="max-w-4xl mx-auto px-4 md:px-6 py-6 md:py-8">
        <h1 className="text-xl md:text-2xl font-light text-gray-900 dark:text-white tracking-wide mb-6 md:mb-10">{t("account.myAccount")}</h1>
          ...
          <div className="md:col-span-1">
            <div className="bg-gray-50 dark:bg-white/5 rounded-sm p-6">
              ...
              <div className="mt-6 pt-4 border-t border-gray-200 dark:border-white/10 space-y-3">
                <Link to="/" className="block text-xs text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">{t("account.shop")}</Link>
                <button onClick={() => { setEditing(!editing); setChangingPassword(false); setFormErrors({}); }} className="block text-xs text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors cursor-pointer">{editing ? t("account.cancelEdit") : t("account.editProfile")}</button>
                <button onClick={() => { setChangingPassword(!changingPassword); setEditing(false); setFormErrors({}); }} className="block text-xs text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors cursor-pointer">{changingPassword ? t("common.cancel") : t("account.changePassword")}</button>
                <button onClick={() => { logout(); navigate("/"); }} className="block text-xs text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors cursor-pointer">{t("common.signOut")}</button>
              </div>
            </div>
          </div>
          ...
          <div className="md:col-span-2">
            {editing ? (
              <div className="bg-gray-50 dark:bg-white/5 rounded-sm p-6 mb-6">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white uppercase tracking-wider mb-4">{t("account.editProfileHeading")}</h3>
                <form onSubmit={handleProfileUpdate} className="space-y-4">
                  <div>
                    <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">{t("common.fullName")}</label>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">{t("common.email")}</label>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">{t("common.phone")}</label>
              ...
              <div className="bg-gray-50 dark:bg-white/5 rounded-sm p-6 mb-6">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white uppercase tracking-wider mb-4">{t("account.changePassword")}</h3>
                <form onSubmit={handlePasswordChange} className="space-y-4">
                  <div>
                    <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">{t("account.currentPassword")}</label>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">{t("account.newPassword")}</label>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">{t("account.confirmNewPassword")}</label>
            ...
            <h2 className="text-sm font-semibold text-gray-900 dark:text-white uppercase tracking-wider mb-4">{t("account.orderHistory")}</h2>
            {loading ? (
              ...
              <div className="text-center py-12 bg-gray-50 dark:bg-white/5 rounded-sm">
                ...
                <p className="text-sm text-gray-400 dark:text-gray-500 mb-4">{t("common.noOrdersYet")}</p>
                  ...
                  return (
                    <div key={order.id} className="border border-gray-100 dark:border-white/10 rounded-sm p-5">
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          <p className="text-sm font-medium text-gray-900 dark:text-white">{t("account.orderNumber", { id: order.id })}</p>
                        ...
                        <span className={`inline-flex px-2.5 py-1 text-[11px] font-semibold rounded-full ring-1 ${colors.bg} ${colors.text} ${colors.ring}`}>
                          {t(`account.status${(order.status || "pending").charAt(0).toUpperCase() + (order.status || "pending").slice(1)}`)}
                        </span>
                      </div>
                      <div className="mb-4">
                        <StatusTracker status={order.status || "pending"} t={t} />
                      </div>
                      <div className="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-white/5">
                        <div className="flex items-center gap-3">
                          <p className="text-xs text-gray-500 dark:text-gray-400">{itemCount} {itemCount !== 1 ? t("common.items") : t("common.item")}</p>
